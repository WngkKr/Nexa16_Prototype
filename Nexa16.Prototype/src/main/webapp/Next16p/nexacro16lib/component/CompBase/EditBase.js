...
if (!nexacro.InputEventInfo)
{
    ...
    _pEditInputType._parse = function (type)
    {
        if (type)
        {
            ...
            {
                var regex = nexacro._EditInputType.MAP[arrType[i]];
                if (regex)
                {
                    bufferType += regex;
                }
            ...
            for (var i = 0; i < lenFilter; i++)
            {
                var regex = nexacro._EditInputType.MAP[arrTypeFilter[i]];
                if (regex)
                {
    ...
    nexacro._EditMaskTypeNumber._put_thousand_seperator = function (text, grouping)
    {
        ...
        if (grouping.length > 1)
        {
            ...
            // /\B(?=(\d{3})+(?!\d))/g
            var reg = new RegExp("(\\d+)(\\d{" + group + "})($|\\.\\d+)");
            if (reg.test(text))
            {
        ...
        {
            var reg = new RegExp("\\B(?=(\\d{" + group + "})+(?!\\d))", "g");
            if (reg.test(text))
            {
    ...
    _pMaskTypeNumber._parseMask = function (mask)
    {
        ...
        {
            var empty_value = "";
            for (var i = 0; i < this._int_digits_min; i++)
                empty_value += '0';
        ...
        // decimal max digit
        this._dec_digits_max = dec_part.length;
    ...
    _pMaskTypeNumber.applyMask = function (value)
    {
        ...
        {
            if (bfillzero) return this._masked_empty_text;
            return "";
        }
        ...
        if (sign_char != '')
        {
            for (var i = 0; i < this._sign_sep_by_space; i++)
                masked_text += ' ';
        }
    ...
    _pMaskTypeString._parseMask = function (mask)
    {
        ...
        var bQuote = false, bEscape = false;
        var nBrace = 0;
    ...
    _pMaskTypeString.applyMask = function (value)
    {
        ...
        while ((mask = mask_buf[bufpos]))
        {
            ...
            else
            {
                ...
                {
                    ch = eval(val.replace("*", "\"" + ch + "\""));
                    char_buf[bufpos] = ch;
                    result_buf.push('*');
                }
                else
                {
                    ch = eval(val.replace("$1", "\"" + ch + "\""));
                    char_buf[bufpos] = ch;
                    result_buf.push(ch);
                }
            }
    ...
    _pMaskTypeString.arrangeMask = function (input_text, begin, end)
    {
        ...
        if (end < mask_len)
        {
            ...
            {
                over_cnt = -(over_cnt);
                for (var i = 0; i < over_cnt; i++)
                {
                    var pos = end + i;
                    var val = value_buf[pos];
                    if (val == "")
                    {
    ...
    _pMaskTypeDate._parseDate = function (date)
    {
        ...
        this._dateObj = new Date(y, parseInt(M) - 1, d);
    };
        ...
        do
        {
            ...
            else
            {
                if (str)
                {
                    ...
                    {
                        //updateinfos.splice(0, 0, { pos: pos, type: type});
    ...
    _pMaskTypeDate.applyMaskSpin = function (caretPos, diff)
    {
        ...
        var intVal = parseInt(tmpStr, 10) + diff;
        ...
        for (i = 0; i < changeidx_buf.length; i++)
        {
    ...
    _pMaskTypeDate.isFilterChar = function (ch, pos)
    {
        ...
        {
            nDate = parseInt(M);
            if (nDate > 12) return true;
        }
        if (d)
        {
            nDate = parseInt(d);
            var maxDay = this.getEndDay(y, M);
            if (nDate > maxDay) return true;
        }
        if (h)
        {
            nDate = parseInt(h);
            if (nDate > 24) return true;
        }
        if (m)
        {
            nDate = parseInt(m);
            if (nDate > 60) return true;
        }
        if (s)
        {
            nDate = parseInt(s);
    ...
    _pMaskTypeDate.changeNormalizeValue = function (v)
    {
        ...
        if (!nexacro._isNull(y)) str += y.padLeft(4, "0");
        if (!nexacro._isNull(M)) str += M ? M.padLeft(2, "0") : "01";
        if (!nexacro._isNull(d)) str += d ? d.padLeft(2, "0") : "01";
        if (!nexacro._isNull(h)) str += h.padLeft(2, "0");
        if (!nexacro._isNull(m)) str += m.padLeft(2, "0");
        if (!nexacro._isNull(s)) str += s.padLeft(2, "0");
        if (!nexacro._isNull(ss)) str += ss.padLeft(3, "0");
    ...
    _pMaskTypeDate._appliedMaskString = function (info, text)
    {
        ...
        var intVal = val ? parseInt(val) : 0;
        var jsDate = new Date(date[0], parseInt(date[1]) - 1, date[2]);
