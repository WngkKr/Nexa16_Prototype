...
{    
    ...
    nexacro.SnapManager = function (target)
    {
        ...
        this._snap_point = { "x": this._snap_length + 1, "y": this._snap_length + 1 };
    };
    ...
    {    
        switch (info_type)
        {
            case nexacro.SnapInfo.INFO_HORI:    this._arr_x.push(info);     break;
            case nexacro.SnapInfo.INFO_VERT:    this._arr_y.push(info);     break;
            case nexacro.SnapInfo.INFO_GUIDE:   this._arr_guide.push(info);     break;
        }
    };
    ...
    _pSnapManager.add_dot_info = function ()
    {
        ...
        for (var i = 0 ; i < nexacro.UXFORM_CANVAS_SIZE_X ; i += dist)
        {
            ...
            {
                // step이 있을 경우 step마다 dot 위치가 초기화 된다.
    ...
    _pSnapManager.add_components_info_for_guideline = function (obj, pos, hori, stepcount, stepwidth)
    {
        ...
        if (comps)
        {
            ...
            for (var i = 0 ; i < count ; i++)
            {
                ...
                // guideline와 붙어있는 component는 move / resize 된다.
                if (snap_comp)
                {
    ...
    {
        // Resize일 경우 Track Type에 따라 Check 기준이 다르다
        switch (cursor_type)
        {
    ...
    _pSnapManager._make_snap_draw_info = function (rc, hori, cursor_type)
    {
        ...
        var offset = 0;
//         UX_SELECT_INFO* pInfo = m_pDesignWindow->GetSelectItemInfo(0);
//         if (pInfo && pInfo->pSelectItem == m_pDesignWindow->GetBaseItem() && pInfo->nSelectedStepIndex > 0)
//         {
//             // 다른 step에서 BaseItem을 Resize 하였다.
//             // Snap Line을 보정 해주어야 한다.
//             UX_FORMRECT rcBase = m_pDesignWindow->GetComponentRect(pInfo->pSelectItem);
...
//         }
        ...
        for (var i = 0 ; i < count ; i++)
        {
            ...
            if (info)
            {
                for (var index = 0 ; index < 5 ; index++)
                {
                    ...
                    {
                        //trace("_pSnapManager._make_snap_draw_info : " + pos[index] +","+ info.pos);
                        // Component와 Component끼리일 경우. GuideLine을 길게 붙여준다.
    ...
    _pTrackerLayer.check_cursor = function (x, y)
    {
        ...
        else
        {
                ...
                for (var i = 0 ; i < count ; i++)
                {
                    ...
                    for (var j = 0 ; j < 8 ; j++)
                    {
                        ...
                        if (rcTracker[0] < x && rcTracker[0] + rcTracker[2] > x && rcTracker[1] < y && rcTracker[1] + rcTracker[3] > y &&
                            ...
                            //&& pInfo->enTrackerType[i] != UX_TRACKER_POSITION_BASE
                            )
                        {
                            cursor = j;
                            break;
            ...
            else
            {
                ...
                if (_check_base_tracker(this._get_tracker_rect(nexacro._mouse_cursor_r, rect))) cursor = nexacro._mouse_cursor_r;
                else if (_check_base_tracker(this._get_tracker_rect(nexacro._mouse_cursor_rb, rect))) cursor = nexacro._mouse_cursor_rb;
                else if (_check_base_tracker(this._get_tracker_rect(nexacro._mouse_cursor_b, rect))) cursor = nexacro._mouse_cursor_b;
            }
        }
    ...
    _pTrackerLayer._get_tracker_rect = function (v, rect)
    {
        ...
        // Tracker size : 6x6
        // Tracker는 object와 1px 떨어져서 생성
        if (v == nexacro._mouse_cursor_lt) rc	= [left - 7, top - 7, left - 1, top - 1];
        if (v == nexacro._mouse_cursor_t) rc	= [width_harf - 3, top - 7, width_harf + 3, top - 1];
        if (v == nexacro._mouse_cursor_rt) rc	= [right + 1, top - 7, right + 7, top - 1];
        if (v == nexacro._mouse_cursor_l) rc	= [left - 7, height_harf - 3, left - 1, height_harf + 3];
        if (v == nexacro._mouse_cursor_r) rc	= [right + 1, height_harf - 3, right + 7, height_harf + 3];
        if (v == nexacro._mouse_cursor_lb) rc	= [left - 7, bottom + 1, left - 1, bottom + 7];
        if (v == nexacro._mouse_cursor_b) rc	= [width_harf - 3, bottom + 1, width_harf + 3, bottom + 7];
        if (v == nexacro._mouse_cursor_rb) rc = [right + 1, bottom + 1, right + 7, bottom + 7];
    ...
    _pTrackerLayer._make_draw_info = function ()
    {
        ...
        var comps = [];
        if (this._move_comps) comps = comps.concat(this._move_comps);
        if (this._resize_comps) comps = comps.concat(this._resize_comps);
        if (this._fixed_comps) comps = comps.concat(this._fixed_comps);
    ...
    _pTrackerLayer._make_snap_draw_info = function ()
    {
        ...
        for (var i = 0 ; i < count ; i++)
        {
            ...
            if (info.length > 0) info_list.push(info);
    ...
    _pTrackerLayer._drag_move = function (x, y)
    {
        ...
        for (var i = 0 ; i < count ; i++)
        {
            ...
            // fixed component일 경우 모든 step에 그려준다.
            if (stepcount < 2)
            {
                // 현재 Layout은 Step이 1페이지임.
                continue;
            }
    ...
    _pTrackerLayer._drag_resize = function (x, y)
    {
        ...
        while (comp)
        {
            ...
            {
                // form의 0,0 기준 좌표이기 때문에
                // form 일경우에만.. border 영역 만큼 보정해주어야 한다
            ...
            case nexacro._mouse_cursor_create_component:
                {
                    ...
                    var compid = designform.createComponentByRect(
                        nexacro._design_mode, parentid,
    ...
    _pTrackerLayer.execKeyDown = function(v)
    {
        ...
        switch (v)
        {
            ...
            case nexacro.Event.KEY_TAB:
                {
                    // 같은 레벨에서 다음 zorder 항목 탐색. 맨 마지막일 경우 첫번째 항목
                    // 같은 레벨이 없을 경우 이동 안함
                    var pivot = designform._selected_components.get_pivot();
                    if (pivot)
                    {
        ...
        if (rootobj)
        {
            ...
            for (var i = 0 ; i < count ; i++)
            {
                ...
                if (!(comp instanceof nexacro.Tabpage))
                {
                    ...
                    info += (rect[0] + ",");    // left
                    info += (rect[1] + ",");    // top 
                    info += ((comp._taborder ? comp._taborder : "0") + ",");    // taborder
                    info += i;  // zorder
    ...
    //===============================================================
    // nexacro.DesignForm : DesignWindow Event
    //===============================================================
    ...
    {        
        ...
        // step 1. base component 정보
        if (nexacro._design_option_snap_component)
        {
            snap_manager.add_base_info();
        }        

        // step 2. guide line 정보
    ...
    _pDesignForm.GetBaseRect = function (apply_border, scale)
    {
        ...
        if (apply_border)
        {
            ...
            if (border_width > 0)
            {
                ...
                {
                    // form은 left, top 고정이므로 right, bottom에서 2 * border 를 제거
    ...
    _pDesignForm.getStartPos = function (parent)
    {
        ...
        return {"x":parseInt(rect[0]) + border_width, "y":parseInt(rect[1]) + border_width};
    };
    ...
    _pDesignForm.setRoot = function (left, top)
    {
        ...
        if (this._real_root_left < 10) this._real_root_left = 10;
        if (this._real_root_top < 10) this._real_root_top = 10;
    ...
    _pDesignForm.moveRoot = function (left, top, set)
    {
        ...
        if (root_left < 10) root_left = 10;
        if (root_top < 10) root_top = 10;
