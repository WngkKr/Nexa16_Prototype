...
if (!nexacro.WebBrowser) 
{
	...
	_pWebBrowser._accessibility_role = "webbrowser";
	...
	_pWebBrowser._getDlgCode = function (keycode, altKey, ctrlKey, shiftKey)
	{
	    ...
	    if (_ifrm_elem)
	    {
	        if ((!_ifrm_elem || (_ifrm_elem._killfocus_flag == true && keycode == nexacro.Event.KEY_UP)) || keycode == nexacro.Event.KEY_DOWN)
	    ...
	    else if (nexacro._enableaccessibility)
	    {
	        ...
	        if (ifrm_elem)
	        {
	            if (keycode == E.KEY_UP)
	            {
	                ...
	                if (ifrm_elem._killfocus_flag == true)
	                {
	                    ifrm_elem._killfocus_flag = false;
	                }
	                else
	                {
	                    try
	                    {
	                        ifrm_elem.handle.contentDocument.body.focus();
	                    }
	                    catch (e)
	                    {
	                    }
	...
	_pWebBrowser.on_apply_accessibility = function ()
	{
		...
		this._setAccessibilityOutfocusAction(this.accessibilityaction);
	};
    ...
    _pWebBrowser.callMethod = function (func)
    {
        ...
        if (ifrm_elem)
        {
            nexacro._WebBrowserPluginElement.prototype.callMethod.apply(ifrm_elem, arguments);
        }
	};
    ...
    {
        var action = this.accessibilityaction;
        ...
        if (action)
        {
            ...
            if (_ifrm_elem)
            {
                if (_ifrm_elem._prev_outfocus_message_elem)
                    _ifrm_elem._prev_outfocus_message_elem.setElementText(action);
                if (_ifrm_elem._next_outfocus_message_elem)
                    _ifrm_elem._next_outfocus_message_elem.setElementText(action);
            }
        }
    };
