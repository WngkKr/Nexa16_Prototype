...
if (!nexacro.Sketch)
{
    ...
    _pSketch.on_create_contents = function ()
    {
        ...
        if (control_elem)
        {
            ...
            var sketchtextarea = new nexacro.SketchEditControl("sketchtextarea", "absolute", this._default_edit_x, this._default_edit_y,
    ...
    */
    ...
    _pSketch.on_apply_text = function (text)
    {
        ...
        if (control_elem)
        {
            ...
            if (expr && expr.length > 0)
            {
                ...
                parser = null;
                exprfn = nexacro._createInlineFunc(this._checkExprFunction(conv_expr), []);

                if (exprfn)
                {
                    try
                    {
                        var val = nexacro._toString(exprfn.call(this));
    ...
    _pSketch._checkExprFunction = function (conv_expr)
    {
        try
        {
            eval(conv_expr);
        ...
        catch (e)
        {
            try
            {
                eval(this._const_form + this._const_dot + conv_expr);
    ...
    */
    ...
    _pSketch.saveSketchToFile = function (fileName, fileType, option)
    {
        ...
        if (nexacro._Browser == "Runtime")
        {
            if (rtn)
            {
                var handleid = nexacro.System.saveToImageObject(this);
                this._set_value(handleid);
    ...
    */
    ...
    {
        if (elem)
            elem.clearRect(0, 0, this.width, this.height);
    };
    ...
    // ==============================================================================
    nexacro.SketchEditControl = function (id, position, left, top, width, height, right, bottom, parent)
    {
