...
public class SampleController {
	...
	public NexacroResult selectPaging(@ParamDataSet(name = "ds_search", required = false) SampleVO searchVO) {
        ...
        SampleVO localSearchVO = searchVO;

        if(localSearchVO == null) {
        	localSearchVO = new SampleVO();
        }
        
        localSearchVO.setPageUnit(10);
        localSearchVO.setPageSize(10);

        
    	PaginationInfo paginationInfo = new PaginationInfo();
    	paginationInfo.setCurrentPageNo(localSearchVO.getPageIndex());
    	paginationInfo.setRecordCountPerPage(localSearchVO.getPageUnit());
    	paginationInfo.setPageSize(localSearchVO.getPageSize());

    	localSearchVO.setFirstIndex(paginationInfo.getFirstRecordIndex());
    	localSearchVO.setLastIndex(paginationInfo.getLastRecordIndex());
    	localSearchVO.setRecordCountPerPage(paginationInfo.getRecordCountPerPage());

    	List<SampleVO> sampleList = sampleService.selectSamplePaging(localSearchVO);

    	int totalCount = sampleService.selectSampleCount(localSearchVO);
    ...
    public NexacroResult test(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("debug_message : nexacro supported parameter types..");
        }
