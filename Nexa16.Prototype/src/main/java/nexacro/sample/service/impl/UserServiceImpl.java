...
public class UserServiceImpl extends EgovAbstractServiceImpl implements UserService {
    ...
    //Map처리추가 
    @Override
    public void modifyMultiUserMap(List<Map> modifyMapList) {

        int size = modifyMapList.size();
        
        for (int i=0; i<size; i++) {
        	
        	Map map = modifyMapList.get(i);
        	
            int rowType = ((Integer) map.get(DataSetRowTypeAccessor.NAME)).intValue(); 
