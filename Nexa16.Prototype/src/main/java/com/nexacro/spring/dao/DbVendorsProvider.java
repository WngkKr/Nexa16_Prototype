...
<bean id="dbmsProvider" class="com.nexacro.spring.dao.DbVendorsProvider">
	...
	*/
	...
	private static final Log LOGGER = LogFactory.getLog(DbVendorsProvider.class);
	...
	public Dbms getDbms(Connection conn) {
		if (conn == null)
			throw new PersistenceException("Connection cannot be null");
		...
		try {
			...
			if (this.dbvendors != null) {
				...
				if( dbms == null) {
					throw new PersistenceException("Not exist "+productName + " [Check context-nexacro.xml]");
				}
				return dbms; // no match, return null
			}
		} catch (SQLException sqle) {
			LOGGER.error("Could not get a Dbms ProductName", sqle);
		}
	...
	public Dbms getDbms(DataSource dataSource) {
		if (dataSource == null) {
			throw new PersistenceException("dataSource cannot be null");
		}
		...
		try {
			...
			LOGGER.error("Could not get a Dbms from dataSource", e);
		} finally {
			if(connection != null) {
				try { 
					connection.close(); 
