...
public class NexacroMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private final Logger logger = LoggerFactory.getLogger(NexacroMethodArgumentResolver.class);
    private final Logger performanceLogger = LoggerFactory.getLogger(NexacroConstants.PERFORMANCE_LOGGER);
...
//    }
    ...
    private void addConvertListeners(NexacroConverter converter) {
        if(this.convertListeners == null || this.convertListeners.isEmpty()) {
            return;
        }
        
        Iterator<NexacroConvertListener> convertListener = this.convertListeners.iterator();
        
        while(convertListener.hasNext()) {
        	NexacroConvertListener listener = convertListener.next();
