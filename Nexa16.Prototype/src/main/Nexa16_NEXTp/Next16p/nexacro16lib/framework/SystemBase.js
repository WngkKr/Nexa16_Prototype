...
if (!nexacro._Init_systembase)
{
    ...
    nexacro.Version = "16";
    nexacro._framework_libpath = "./nexacro16lib";
    ...
    {
        return (nexacro._isNull(v) || v === "") ? def : v;
    };
    // undefined,null,NaN
    nexacro._parseInt = function(v, def)
    {
        if (nexacro._isNull(v)) return def;
        var ret = parseInt(v);
    ...
    */
    ...
    nexacro.Touch.prototype = _pTouchInput;
    ...
    _pTouchSession.onaction = function (name, status, data)
    {
        ...
        /*
        else if (name == "pinch") //Currently, pinch doesn't support.
        {
            switch (status)
            {
                case nexacro.TouchAction._STATUS_START:
    ...
    _pTouchSession.calcActionData = function (input)
    {
            ...
            (type == nexacro.Touch._TOUCH_END && leadTime <= this.opt_interval)))
        {
            if (prev_velocity_data)
            {
                ...
                var v = this.getVelocity(velocity_deltaTime, velocity_distanceX, velocity_distanceY);
                deltaVelocityX = v.x;
                deltaVelocityY = v.y;
    ...
    _pTouchSession.getCenter = function (touches)
    {
        ...
        {
            var touch = touches[0];
            return { x: Math.round(touch.windowx), y: Math.round(touch.windowy) };
        }
    ...
    nexacro.TouchAction_Slide = function ()
    {
        ...
        this._fling_timing_function = new nexacro._CubicBezier(0, 0.3, 0.6, 1);
    };
    ...
    _pTouchActionSlide.opt_threshold = 10;
    _pTouchActionSlide.opt_velocity = 0.15;     // for fling
    _pTouchActionSlide.opt_friction = 0.998;    // for fling
    ...
    _pTouchActionSlide._on_action_fling = function (status)
    {
        ...
        this._fling_start_time = Date.now();
    };
    ...
    _pTouchActionSlide._on_fling_frame = function ()
    {
        ...
        // Interpolation
        var leadTime = Date.now() - this._fling_start_time;
    ...
    _pTouchActionPress._analysysGesture = function ()
    {
        ...
        if (passOption)
        {
            if (evtType == nexacro.Touch._TOUCH_START)
            {
                ...
                this._timer = nexacro._setSystemTimer(win_handle, timer_handler, this.opt_time);
            }
            else if (evtType == nexacro.Touch._TOUCH_END)
            {
                this.reset();
    ...
    _pTouchActionTap.opt_distance = 9;      // for tap
    ...
    {
        // mordern Browser support Base64 Encode Functions
        nexacro.base64Encode = function (input)
        {
    ...
    else
    {
        (function ()
        {
            ...
            nexacro.base64Decode = function (input)
            {
                ...
                input = input.replace(re_not_base64Chars, "");
    ...
    nexacro._updateCSSMaps = function (cssmap, updatemap, selfitem, parentitem )
    {
    	if (parentitem == "self")
    	{
    		for (var item in updatemap)
    		{	
    			var mapitem = updatemap[item];
    			cssmap[item] = mapitem;
    		}
    	}
    	else
    ...
    nexacro._CommunicationItem = function (path, type, bcache, last_modified, version)
    {
        ...
        this._addCookieFromEnvVariables();
    ...
    //__pCommunicationItem._addCookieFromGlobalvariables = function ()
    //{
    //    var app = nexacro.getApplication();
    //    var cookievar = app._cookie_variables;
    //    var cookievarCnt = 0;
    //    if (cookievar)
    ...
    //        if (curCookie != app[cookieid])
    //            nexacro._setCookie(cookieid, app[cookieid]);
    //    }
    //};

    __pCommunicationItem._addCookieFromEnvVariables = function ()
    {
    ...
    __pCommunicationItem._addCookieToEnvVariable = function (cookieStr)
    {    	
    	...
    	if (cookieStr)
    	{
    		...
    		for (var i = 0; i < cookievarCnt; i++)
    		{
    			...
    			if (nexacro._addcookietovariable)
    				nexacro.setEnvironmentVariable(cookieid, cookievalue, 3);    						
            }
        }
    };
    ...
    __pCommunicationItem.on_error = function (errstatus, fireerrorcode, returncode, locationuri)
    {
        delete nexacro._CommunicationManager[this.path];
    ...
    __pCommunicationItem.on_error_image = function (width, height, handle, errstatus, fireerrorcode, returncode, locationuri)
    {
        delete nexacro._CommunicationManager[this.path];
        ...
        if (n > 0)
        {
            for (var i = 0; i < n; i++)
            {
                ...
                if (target._is_alive !== false)
                {
                    item.callback.call(target, this.path, width, height, handle, errstatus, null, fireerrorcode, returncode, this.path, locationuri);
                }
            }
            callbackList.splice(0, n);
        }
    ...
    nexacro._loadUpdateModule = function (path, target, handler, service, async, type, targetpath, ref, ver, failpass)
    {
        ...
        {
            var loadItem = new nexacro._CommunicationItem(path, "update");
            loadItem.appendCallback(target, handler);
    ...
    nexacro._preloadData = function (path, target, handler, service, svcid, referer, args, async, datatype, compress)
    {
        ...
        if (service && d_cache)
        {
            if (service.cachelevel == "session" || service.cachelevel == "static")
            {
                ...
                {
                    // getDataFromCache
                    var loadItem = new nexacro._CommunicationItem(svcid, "data");
    ...
    nexacro._attachAccessibilityComponentFunctions = function ()
    {
        ...
        _ptype._setAccessibilityAction = function (action)
        {
            var control_elem = this._control_element;
            if (control_elem)
        ...
        _ptype._setAccessibilityActiveDescendant = function (subcomp)
        {
            var control_elem = this._control_element;
            if (control_elem)
                control_elem.setElementAccessibilityActiveDescendant(subcomp.getElement(), subcomp._input_element);
        };
        ...
        _ptype._setAccessibilityStatFlag = function (status, userstatus)
        {
        	if (status == "enable")
        	{
            ...
            else if (this._pushed) // && status == "pushed")
            {
        ...
        _ptype._setAccessibilityStatFocus = function (evt_name)
        {
            if (this._control_element)
            {
                ...
                if (focusdir > -1) // && nexacro._tabkeycirculation == 0)
                {
                    ...
                    if (form)
                    {
                        ...
                        /*
                        var tabcir = true;
                        if (nexacro._tabkeycirculation == 1) tabcir = false;
                        ...
                        */                
                        ...
                        if (current == form._getTabOrderFirst(baccessibility))
                        {
                            ...
                            var app = nexacro.getApplication();
                            _label = current._getAccessibilityReadLabel() + " " + app.accessibilityfirstovertext;
                        }
                        if (current == form._getTabOrderLast(baccessibility))
                        {
                            ...
                            var app = nexacro.getApplication();
                            _label = current._getAccessibilityReadLabel() + " " + app.accessibilitylastovertext;
                        }
                    }
                }
                }
            ...
            var app = nexacro.getApplication();
            app._set_accessibility_history(this);
        };       
    ...
    {        
    	...
    	if (!this._defaultservice)
    	{
    		...
    		nexacro._defaultservice.version = "0.0";
    	}
    ...
    nexacro._getScreen = function (screeninfo)
    {
    	...
    	if (is_landscape && nexacro._OS == "Android")
    	{
    ...
    nexacro._applySelectedScreen = function (curscreen)
    {
    	...
    	else
    	{
    		...
    		{
    			// 100% -> 1.0
    			curzoommin = parseFloat(zoommin.slice(0, zoommin.length - 1)) / 100.0;
    		}
    		else
    		{
    			curzoommin = parseFloat(zoommin);
    		}

    		if (curzoommin >= 1)
    			curzoommin = 1;
    		nexacro._minimum_scale = curzoommin;
    	}
    	...
    	else
    	{
    		...
    		{
    			// 100% -> 1.0
    			curzoommax = parseFloat(zoommax.slice(0, zoommax.length - 1)) / 100.0;
    		}
    		else
    		{
    			curzoommax = parseFloat(zoommax);
    		}

    		if (curzoommax <= 1)
    			curzoommax = 1;
    		nexacro._maximum_scale = curzoommax;
    	}

    	//Autozoom
    	var cur_type = curscreen._cur_type ? curscreen._cur_type : curscreen.type;
    	if (!cur_type)
    	{
    		cur_type = "desktop";
    		switch (nexacro._SystemType.toLowerCase())
    		...
    		curscreen._cur_type = cur_type;
    	}

        // autozoom은 Phone,Tablet인1),2),3) 경우에 지원
        // cur_type case 1) Phone 2) Tablet 3) Phone,Tablet 4) Desktop 
    	if (cur_type == "desktop")
    		return;

        /*android runtime 에서는 screenwidth를 입력하지 않으면 undefined.
          html5에서는 screenwidth를 입력하지 않으면 "". 
          하기의 다른 연산들때문에 맞춰야 하지 않을까...*/
    	//if (curscreen.screenwidth == undefined)
    	//    curscreen.screenwidth = "";
    	...
    	if (curscreen.screenwidth !== undefined)// && nexacro._toBoolean(curscreen.autozoom) === true)
    	{
    		...
    		if (parseInt(curscreen.screenwidth) <= 320 && nexacro._OS == "Android" && nexacro._Browser != "Chrome")
    	...
    	{
    		//trace("screenwidth of screeninfo is undefined");
    			...
    			for (var i = cnt; i > 0; i--)
    			{
    				...
    				if (bCheckSameParent)
    				{
    					...
    					else
    					{
    						...
    						var root_comp = cur_popup._getRootComponent(cur_popup);
